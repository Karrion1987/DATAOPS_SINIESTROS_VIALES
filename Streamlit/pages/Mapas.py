...
hechos_limpios = h_hechos.dropna(subset=['pos x', 'pos y'])
hechos_limpios = hechos_limpios[(hechos_limpios['pos x'] != '.') & (hechos_limpios['pos y'] != '.')]
# Crear un DataFrame para los puntos de interés
puntos_interes = pd.DataFrame({
    'lat': hechos_limpios['pos y'],
    'lon': hechos_limpios['pos x'],
})
